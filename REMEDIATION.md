...
- **First concrete non-US fetcher live:** `brazil_cvm.py` ingests CVM open data
  end-to-end — a real run over DFP 2023 yields ~475 companies with genuine filing
  dates (Petrobras ROE 32.7%, Vale 20.4%), and `pit_panel.as_of("2024-06-30")`
  correctly excludes filings received after that date. Remaining L1/L2/L3 work is
  the *other* markets' fetchers (registry + discipline already in place) and a
  financial-sector account-code variant for CVM (banks/insurers use a different
  taxonomy; `plausible_roe` flags them meanwhile).
